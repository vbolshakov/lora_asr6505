 ...
 * LoRaMAC channels
 */
static ChannelParams_t Channels[EU868_MAX_NB_CHANNELS] = { EU868_LC1, EU868_LC2, EU868_LC3 };
...
static uint8_t CountNbOfEnabledChannels( bool joined, uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx )
{
    ...
    for( i = 0, k = 0; i < EU868_MAX_NB_CHANNELS; i += 16, k++ )
    {
        for( j = 0; j < 16; j++ )
        {
...
void RegionEU868ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( i = 0, chanIdx = EU868_NUMB_DEFAULT_CHANNELS; chanIdx < EU868_MAX_NB_CHANNELS; i+=3, chanIdx++ )
    {
