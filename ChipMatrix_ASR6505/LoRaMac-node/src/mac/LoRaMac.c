...
static void OnRadioRxTimeout( void )
{
    if( LoRaMacDeviceClass != CLASS_C )
    {
        //printf("OnRadioRxTimeout Not Class C go to sleep\n");
        Radio.Sleep( );
    }
    ...
    if( RxSlot == RX_SLOT_WIN_1 )
    {
        ...
        //printf("RadioTimeout in RX_ALOT_WIN_1");
        if( NodeAckRequested == true )
        {
    ...
    {
        //printf("RadioTimeout in RX_ALOT_WIN_2");
        if( NodeAckRequested == true )
        {
...
LoRaMacStatus_t Send( LoRaMacHeader_t *macHdr, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    // Prepare the frame
//    printf("send...Now before prepareframe and size is: %hu, data in buffer is: ", fBufferSize);
//    for(int k=0; k<fBufferSize; ++k)
//	    printf("%c", ((char *)fBuffer)[k]);
//	    printf("\n");
...
//		printf("prepareframe... before encrypt and fBufferSize is: %hu, LoRaMacTxPayloadLen is: %hu, data in buffer is: ", fBufferSize, LoRaMacTxPayloadLen);
//	        for(int k=0; k<fBufferSize; ++k)
//	      		printf("%c", ((char *)fBuffer)[k]);
//	    	printf("\n");
                if( framePort == 0 )
                ...
                {
//		    printf("pktHeaderLen is %hd\n", pktHeaderLen);
//		    printf("%d\n", UP_LINK);
//		    printf("buffer\[0\]:%s\tsize: %hu\tkey:%02x%02x%02x%02x\taddr:%08x\tdir:0x%02x\tsequencecounter:%d\n", (char *)payload, LoRaMacTxPayloadLen, LoRaMacAppSKey[0], LoRaMacAppSKey[1], LoRaMacAppSKey[2], LoRaMacAppSKey[3], LoRaMacDevAddr, UP_LINK, UpLinkCounter);
//                    printf("%d\n", UP_LINK);
...
//	    printf("LoRaMacBufferPktLen is %hd\n", LoRaMacBufferPktLen);
...
LoRaMacStatus_t SendFrameOnChannel( uint8_t channel )
{
    ...
    // Send now
//    printf("sendframeonchannel... before radio.send and LoRaMacBufferPktLen is: %hu, data in buffer is: ", LoRaMacBufferPktLen);
//    for(int k=0; k<LoRaMacBufferPktLen; ++k)
//        printf("%02x", LoRaMacBuffer[k]);
//    printf("\n");
...
{
    //printf("beginning of loramacinit\n");
    GetPhyParams_t getPhy;
    PhyParam_t phyParam;
...
//	    printf("mcpsrequest...Now in MCPS_CONFIRMED and size is: %hu, data in buffer is: ", fBufferSize);
//	    for(int k=0; k<fBufferSize; ++k)
//	      printf("%c", ((char *)fBuffer)[k]);
//	    printf("\n");
            break;
