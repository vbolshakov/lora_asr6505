...
//    printf("addr:%ul\n", address);
//    printf("sequence:%d\n", sequenceCounter);
    //printf("buffer\[0\]:%c\tsize: %hu\tkey:%02x%02x%02x%02x\taddr:%08x\tdir:0x%02x\tsequencecounter:%d\n", buffer[0], size, key[0], key[1], key[2], key[3], address, dir, sequenceCounter);
    uint16_t i;
    ...
    while( size >= 16 )
    {
	   ...
	   //encBuffer[bufferIndex + i] = buffer[bufferIndex + i] ;
        }
        size -= 16;
        bufferIndex += 16;
    }
    ...
    if( size > 0 )
    {
	    ...
	    //printf("buffer:%02x\n", buffer[bufferIndex + i]);
            encBuffer[bufferIndex + i] = buffer[bufferIndex + i] ^ sBlock[i];
	    //encBuffer[bufferIndex + i] = buffer[bufferIndex + i];
	    //printf("buffer:0x%02x\tencBuffer:0x%02x\n", buffer[bufferIndex + i], encBuffer[bufferIndex + i]);
        }
    }
}
