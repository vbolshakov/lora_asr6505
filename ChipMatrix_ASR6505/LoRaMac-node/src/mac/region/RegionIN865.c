 ...
 * LoRaMAC channels
 */
static ChannelParams_t Channels[IN865_MAX_NB_CHANNELS] = {IN865_LC1, IN865_LC2, IN865_LC3};
...
static uint8_t CountNbOfEnabledChannels( bool joined, uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx )
{
    ...
    for( i = 0, k = 0; i < IN865_MAX_NB_CHANNELS; i += 16, k++ )
    {
        for( j = 0; j < 16; j++ )
        {
...
void RegionIN865ApplyCFList( ApplyCFListParams_t* applyCFList )
{
    ...
    for( i = 0, chanIdx = IN865_NUMB_DEFAULT_CHANNELS; chanIdx < IN865_MAX_NB_CHANNELS; i+=3, chanIdx++ )
    {
