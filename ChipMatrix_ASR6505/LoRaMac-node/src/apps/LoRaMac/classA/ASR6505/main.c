...
static void OnTxNextPacketTimerEvent( void )
{
    ...
    if( status == LORAMAC_STATUS_OK )
    {
        ...
        {
            DeviceState = DEVICE_STATE_SEND;//change dev state so dev send data
            NextTx = true;
        }
        else
        {
...
/* TX led init: GPIO set in output */
    ...
    while( 1 )
    {
		...
		//printf("dev send and the TxDutyCycleTime is %d\n", TxDutyCycleTime);
                //printf("dev send and the TxDutyCycleTime is %d\n", TxDutyCycleTime);
		DeviceState = DEVICE_STATE_CYCLE;
                break;
            }
            case DEVICE_STATE_CYCLE://namely set the txnextpackettimer
            {
                DeviceState = DEVICE_STATE_SLEEP;
