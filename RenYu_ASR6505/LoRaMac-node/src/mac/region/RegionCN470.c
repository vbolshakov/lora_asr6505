...
#ifdef CONFIG_RAM_OPT

static uint32_t RegionCN470GetFrequency(uint8_t channel)
{
    return 470300000 + channel * 200000;
}

static uint8_t RegionCN470GetBand(uint8_t channel)
{
    (void)channel;
...
static DrRange_t RegionCN470GetDrRange(uint8_t channel)
{
    DrRange_t dr;
    (void)channel;
    
    dr.Value = ( DR_5 << 4 ) | DR_0;
    return dr;
}
#endif
...
static uint8_t CountNbOfEnabledChannels( uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx )
{
    ...
    for( i = 0, k = 0; i < CN470_MAX_NB_CHANNELS; i += 16, k++ )
    {
        for( j = 0; j < 16; j++ )
        {
            ...
            {
#ifdef CONFIG_RAM_OPT
                if( RegionCommonValueInRange( datarate, RegionCN470GetDrRange(i+j).Fields.Min,
                                              RegionCN470GetDrRange(i+j).Fields.Max ) == false )
                { // Check if the current channel selection supports the given datarate
                    continue;
                }
                if( bands[RegionCN470GetBand(i+j)].TimeOff > 0 )
...
{
#ifdef CONFIG_RAM_OPT
    RegionCommonSetBandTxDone( txDone->Joined, &Bands[RegionCN470GetBand(txDone->Channel)], txDone->LastTxDoneTime );
#else
...
#ifdef CONFIG_RAM_OPT
    int8_t txPowerLimited = LimitTxPower( txConfig->TxPower, Bands[RegionCN470GetBand(txConfig->Channel)].TxMaxPower, txConfig->Datarate, ChannelsMask );
#else    
...
#ifdef CONFIG_RAM_OPT
    Radio.SetChannel( RegionCN470GetFrequency(txConfig->Channel));
#else    
...
#ifdef CONFIG_RAM_OPT
    printf("TxConfig: freq: %lu, sf: %d\r\n", RegionCN470GetFrequency(txConfig->Channel), (int)phyDr);
#else
    printf("TxConfig: freq: %lu, sf: %d\r\n", Channels[txConfig->Channel].Frequency, (int)phyDr);
#endif    
    return true;
}
...
#ifdef CONFIG_RAM_OPT                   
                    ( (linkAdrParams.ChMaskCtrl * 16 + i) >= CN470_MAX_NB_CHANNELS) )
#else
...
#ifdef CONFIG_RAM_OPT    
    calcBackOffParams.BandIndex = RegionCN470GetBand(calcBackOff->Channel);
#else    
...
{
#ifdef CONFIG_RAM_OPT
    int8_t txPowerLimited = LimitTxPower( continuousWave->TxPower, Bands[RegionCN470GetBand(continuousWave->Channel)].TxMaxPower, continuousWave->Datarate, ChannelsMask );
    int8_t phyTxPower = 0;
    uint32_t frequency = RegionCN470GetFrequency(continuousWave->Channel);
#else  
