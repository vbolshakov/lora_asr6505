 ...
 * LoRaMAC channels
 */
static ChannelParams_t Channels[AS923_MAX_NB_CHANNELS] = { AS923_LC1, AS923_LC2 };
...
static uint8_t CountNbOfEnabledChannels( bool joined, uint8_t datarate, uint16_t* channelsMask, ChannelParams_t* channels, Band_t* bands, uint8_t* enabledChannels, uint8_t* delayTx )
{
    ...
    for( i = 0, k = 0; i < AS923_MAX_NB_CHANNELS; i += 16, k++ )
    {
        for( j = 0; j < 16; j++ )
        {
...
{
    uint8_t i = 0, chanIdx = AS923_NUMB_DEFAULT_CHANNELS;
    ...
    for( i = 0, chanIdx = AS923_NUMB_DEFAULT_CHANNELS; chanIdx < AS923_MAX_NB_CHANNELS; i+=3, chanIdx++ )
    {
...
LoRaMacStatus_t RegionAS923NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    {
        for( i = 0, j = randr( 0, nbEnabledChannels - 1 ); i < AS923_MAX_NB_CHANNELS; i++ )
        {
...
bool RegionAS923ChannelsRemove( ChannelRemoveParams_t* channelRemove  )
{
    ...
    memset(&Channels[id], 0, sizeof(ChannelParams_t));
